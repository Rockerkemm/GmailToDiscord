...
CREDENTIALS_FILE = os.getenv("CREDENTIALS_FILE", "credentials.json")  # Path to OAuth credentials JSON
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
            ...
            if not os.path.exists(CREDENTIALS_FILE):
                error_msg = f"OAuth credentials file not found: {CREDENTIALS_FILE}"
                print(f"{error_msg}")
                raise FileNotFoundError(error_msg)
            
            # Instead of trying to run OAuth flow, provide instructions
            print("No valid OAuth token found.")
            raise Exception("OAuth token required - please generate token.json on a local machine. Refer to README")
